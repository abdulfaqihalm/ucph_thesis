...
def train(model: torch.nn.Module, train_loader: DataLoader, test_loader: DataLoader, epochs: int,
    ...
    logger = csv.DictWriter(logfile, fieldnames=["epoch", "train_loss", "val_control_loss", "val_control_rmse", "val_control_mse", "val_control_mae",
                            "val_control_pearson_corr", "val_control_R2", "val_case_loss", "val_case_rmse", "val_case_mse", "val_case_mae", "val_case_pearson_corr", "val_case_R2"])
    logger.writeheader()
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch, data in enumerate(epoch_iterator):
            ...
            meth_pred_val = model(seq)
            control_weight = data["meth_control_weight"].to(
                device, non_blocking=True)
            case_weight = data["meth_case_weight"].to(
                device, non_blocking=True)
            ...
            loss1 = loss_fn(meth_pred_val[:, 0],
                            meth_true_val[:, 0], control_weight)
            loss2 = loss_fn(meth_pred_val[:, 1],
                            meth_true_val[:, 1], case_weight)
            loss = loss1 + loss2
            loss.backward()
            optimizer.step()
            # Total of average loss from each samples in a batch. Huberloss reduction is mean by default
        ...
        logger.writerow({"epoch": epoch, "train_loss": train_loss, "val_control_mse": metrics['mse'][0], "val_control_rmse": metrics['rmse'][0], "val_control_mae": metrics['mae'][0], "val_control_pearson_corr": metrics['pearson_corr'][
                        0], "val_control_R2": metrics['R2'][0], "val_case_mse": metrics['mse'][1], "val_case_rmse": metrics['rmse'][1], "val_case_mae": metrics['mae'][1], "val_case_pearson_corr": metrics['pearson_corr'][1], "val_case_R2": metrics['R2'][1]})
    ...
    if tensorboard_writer and ("config" in kwargs) and fold == 1:
        tensorboard_writer.add_hparams(hparam_dict=kwargs["config"], metric_dict={"val_control_mse": metrics['mse'][0].item(), "val_case_mse": metrics['mse'][1].item(), "val_control_pearson_corr": metrics['pearson_corr'][0].item(
        ), "val_case_pearson_corr": metrics['pearson_corr'][1].item(), "val_control_R2": metrics['R2'][0].item(), "val_case_R2": metrics['R2'][1].item()}, run_name=f"{fold_info}hparam")
    tensorboard_writer.close()
    ...
    if plts:
        for name, plt in plts.items():
            tensorboard_writer.add_figure(
                f"{fold_info}LSTM Weight/{name}", plt.gcf())
        tensorboard_writer.close()
    logfile.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fasta_file", help="Fasta file for inference. Optional, needed for inference", required=False)
    parser.add_argument(
        "--model_file", help="Model file for inference. Optional, needed for inference", required=False)
    parser.add_argument(
        "--num_epochs", help="Max number of epochs", required=True)
    parser.add_argument(
        "--batch_size", help="Number of batch size", default=100, type=int)
    parser.add_argument(
        "--save_dir", help="Directory for saving outputs", required=True)
    ...
    parser.add_argument("--adam_beta1", default=0.9,
                        type=float, help="Beta1 for Adam optimizer.")
    parser.add_argument("--adam_beta2", default=0.999,
                        type=float, help="Beta2 for Adam optimizer.")
    ...
    parser.add_argument("--plot", type=str2bool, nargs='?',
                        const=True, default=False, help="Plot loss or not [y/n]")
    parser.add_argument("--suffix", default="", help="Suffix for output files")
    parser.add_argument(
        "--patience", help="Number of patience for training", default=2, type=int)
    parser.add_argument("--weighted_loss", type=str2bool,
                        nargs='?', default=False, help="Weighted Loss[y/n]")
    parser.add_argument("--tensorboard_writer", type=str2bool,
                        nargs='?', default=True, help="Tensor Board Logging[y/n]")
    parser.add_argument("--kaiming_init", type=str2bool, nargs='?', default=False,
    ...
    if torch.cuda.is_available():
        ...
        gpu_name = subprocess.check_output(
            ['nvidia-smi', '--query-gpu=gpu_name', '--format=csv']).decode().split('\n')[1]
    ...
    if args.mode == "train":
        ...
        logging.info(
            f"m6A_info: {args.m6A_info}, add_promoter: {args.add_promoter}, embedding: {args.embedding}, target: dual_outputs")
        ...
        for fold in folds:
            ...
            train_dataset = SequenceDatasetDual(seq_fasta_path=seq_fasta_train_path, meta_data_path=meta_data_train_json_path, prom_seq_fasta_path=promoter_fasta_train_path,
                                                m6A_info=args.m6A_info, m6A_info_path=m6A_info_train_path, transform=args.embedding, path_to_embedding=embedding_file)
            test_dataset = SequenceDatasetDual(seq_fasta_path=seq_fasta_test_path, prom_seq_fasta_path=promoter_fasta_test_path,  meta_data_path=meta_data_test_json_path,
                                               m6A_info=args.m6A_info, m6A_info_path=m6A_info_test_path, transform=args.embedding, path_to_embedding=embedding_file)
            ...
            train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                                      num_workers=loader_worker, persistent_workers=True, pin_memory=True)
            test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False,
                                     num_workers=loader_worker, persistent_workers=True, pin_memory=True)
            logging.info(
                f"Number of training batches: {len(train_loader)}, Total training sample: {len(train_loader.dataset)}")
            logging.info(
                f"Number of test batches: {len(test_loader)}, Total test sample: {len(test_loader.dataset)}")
            ...
            if args.tensorboard_writer:
                tensorboard_writer = create_tensorboard_log_writer(
                    experiment_name=f"{suffix}", model_name=model.__class__.__name__, log_dir=f"{args.save_dir}/tensorboard_logs")
            ...
            if fold == folds[0]:
                ...
                summary(model, verbose=1, col_width=15, input_size=(args.batch_size, 4, input_size),  col_names=[
        ...
        if not args.fasta_file:
            raise ValueError("Fasta file needed for inference")
        if not args.model_file:
            raise ValueError("Model file needed for inference")
