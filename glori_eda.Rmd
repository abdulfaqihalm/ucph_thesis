...
hela_raw_intrsct <- hela_1 |> 
  ...
  dplyr::rename(Strand=Strand.x, Gene=Gene.x, Transcript=Transcript.x) |> 
  dplyr::select(Chr, Sites, Strand, Gene, Transcript, Ratio.x, Ratio.y) |> 
  rowwise() |> 
  mutate(meth_ctrl_mean = mean(c_across(c('Ratio.x', 'Ratio.y')), na.rm=TRUE)) |> 
  ungroup() |> 
...
hela_raw_hypoxia_intrsct <-  hypox_hela_1 |> 
  ...
  dplyr::select(Chr, Sites, Strand, Gene, Transcript, Ratio.x, Ratio.y) |> 
  rowwise() |> 
  mutate(meth_case_mean = mean(c_across(c('Ratio.x', 'Ratio.y')), na.rm=TRUE)) |> 
  ungroup() |> 
  dplyr::select(!c(Ratio.x, Ratio.y))
...
hela_hypoxia_intrsct <- hela_raw_intrsct |> 
  ...
  dplyr::select(c(Chr, Sites, Strand, meth_case, meth_control))
