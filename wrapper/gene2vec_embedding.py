...
def process_sequences(sequences: list[str], embedding: Word2Vec):
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
            results = list(executor.map(gene2vec, [(str(seq.seq), embedding) for seq in sequences]))
    return results
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fasta_file_pattern", help="Fasta file pattern", required=True)
