...
db_file <-"data/analysis/CISBP-RNA/Homo_sapiens.meme"
# We force TOMTOM to work on non-RC since we expect that the nascent RNA will not be reverse-transcribed 
tomtom_result <- memes::runTomTom(unlist(motList), database = db_file, thresh = 0.1, norc="")
...
for(i in 1:dim(tomtom_mathed_result_df)[1]){
  ...
  p <- universalmotif::view_motifs(c(temp_motif, temp_best_match)) + labs(title = paste0("e-val: ", tomtom_mathed_result_df[i,"best_match_eval"])) + theme(axis.title.y = element_blank()) 
  match_plot_list[[i]] <- p 
...
dup_mots <- mot_df |> 
  ...
  dplyr::summarise(fold_count = n_distinct(fold), count = n()) |> 
...
empty <- ggplot()+geom_point(aes(1,1), colour="white")+
         theme(axis.ticks=element_blank(), 
               ...
               axis.text.x=element_blank(), axis.text.y=element_blank(),           
               axis.title.x=element_blank(), axis.title.y=element_blank())
grid.arrange(p_hist_estimate, empty, p, p_hist_icscore, ncol=2, nrow=2, widths=c(6, 1), heights=c(1, 6))
...
```{r}
top_motif_dist_df <- NULL
for(i in 1:NUM_FOLDS){
  temp <- scan_list[[i]] |> 
    dplyr::filter(consensus %in% c(neg_motif, pos_motif)) |> 
    dplyr::rowwise() |> 
    dplyr::mutate(
      m6A_pos = data_split_TEST[[i]][[sequence.i, "start"]],
      gene = data_split_TEST[[i]][[sequence.i, "gene"]],
      gene_type = data_split_TEST[[i]][[sequence.i, "gene_type"]],
      tx_id = data_split_TEST[[i]][[sequence.i, "tx_id"]],
      mot_start = start,
      rel_start = start + m6A_pos - 500,
      rel_end = stop + m6A_pos - 500,
      chr = data_split_TEST[[i]][[sequence.i, "seqnames"]],
      pred_control = valid_data_list[[i]][[sequence.i, "pred_control"]],
      pred_case = valid_data_list[[i]][[sequence.i, "pred_case"]],
      regulation_flag = case_when(pred_case - pred_control  < -0.1 ~ "down",
                                  pred_case - pred_control > 0.1 ~ "up",
                                  .default = "no_change")
    ) |> 
    dplyr::ungroup() |> 
    dplyr::select(motif, consensus, gene,  gene_type, tx_id, chr,  m6A_pos, mot_start, rel_start, rel_end, pred_control, pred_case, regulation_flag)
  top_motif_dist_df <- dplyr::bind_rows(top_motif_dist_df, temp)
}


# top_motif_dist_df |> dplyr::filter(gene_type == "protein_coding", chr != "chrX", chr != "chrY") |> dplyr::group_by(gene) |> dplyr::summarise(n=n()) |> dplyr::ungroup() |> dplyr::arrange(desc(n))
```


```{r}
ggplot(data=top_motif_dist_df |> dplyr::filter(consensus %in% c("UAGCCCC", "WGGUAGG", "AGGUAAG", "HSCGAAU", "GUCSACS", "WSGAAGA")) ) + geom_density(mapping = aes(x=mot_start, color=regulation_flag)) + facet_wrap(vars(consensus))
```
...
```{r}
temp_select_gene <- top_motif_dist_df |> dplyr::filter(gene_type == "protein_coding", chr != "chrX", chr != "chrY") |> dplyr::filter(gene=="VEGFA")
mrna_plot_data |> dplyr::left_join(top_motif_dist_df |> dplyr::filter(gene_type == "protein_coding", chr != "chrX", chr != "chrY")  |> dplyr::select(mot_start, rel_start, rel_end, consensus, m6A_pos), by = join_by(start==m6A_pos))


coba <- mrna_plot_data |> dplyr::left_join(top_motif_dist_df |> dplyr::filter(gene_type == "protein_coding", chr != "chrX", chr != "chrY")  |> dplyr::select(mot_start, rel_start, rel_end, consensus, m6A_pos), by = join_by(start==m6A_pos))

coba |> dplyr::filter(gene=="PHF13")
```

```{r}
coba2 <- coba |> 
    dplyr::mutate(mot_pos_flag = ifelse(strand_gene == "+", 
                                    case_when(rel_start < cds_start ~ "5UTR",
                                              rel_start >= cds_start & rel_start < utr3_start ~ "CDS",
                                              rel_start >= utr3_start ~ "3UTR"),
                                    case_when(rel_start > cds_start ~ "5UTR", 
                                              rel_start <= cds_start & rel_start > utr3_start ~ "CDS",
                                              rel_start <= utr3_start ~ "3UTR"))) |> 
  dplyr::distinct()
...
```{r}
ggplot(data = coba2 |> dplyr::filter(!is.na(rel_start)) |> dplyr::select(consensus, mot_pos_flag, mot_start, rel_start) |> dplyr::distinct()) +
  geom_density(mapping = aes(x = mot_start, color = consensusj))
```
